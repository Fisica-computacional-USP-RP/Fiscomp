...
#
# Para cada metodo:
# - mostramos saida numerica
# - mostramos animacao da mola oscilando
# - mostramos grafico de posicao x(t)
...
# Parametros fisicos
m = 1.0          # massa (kg)
k = 1.0          # constante elastica (N/m)

# Condicoes iniciais
x0 = 1.0         # posicao inicial (m)
...
# --------------------------
def simular_euler():
    # Vetores para guardar posicao e velocidade
    x = np.zeros(N)
    v = np.zeros(N)
    ...
    # Integracao temporal com Euler
    for i in range(N - 1):
        a = -(k / m) * x[i]      # aceleracao da EDO
        v[i + 1] = v[i] + a * dt
        x[i + 1] = x[i] + v[i] * dt
...
# --------------------------
def derivadas(x, v):
    # Sistema equivalente de 1a ordem
    dx_dt = v
    dv_dt = -(k / m) * x
    return dx_dt, dv_dt
...
def desenhar_mola(x_massa, x_parede=-1.2, espiras=12, amplitude=0.08, pontos=220):
    # Posicao final da mola: um pouco antes do centro da massa
    ...
    # Forca ponta inicial e final em y=0 para encostar reto na parede/massa
    ys[0] = 0.0
    ys[-1] = 0.0
...
def animar_mola(nome_metodo, x):
    ...
    # Parede fixa onde a mola esta presa
    ax_mola.axvline(-1.2, color="gray", lw=3)

    # Artistas que serao atualizados na animacao
    linha_mola, = ax_mola.plot([], [], color="tab:blue", lw=2)
    massa, = ax_mola.plot([], [], "o", color="tab:red", ms=14)

    # ----- Painel inferior: grafico de posicao -----
    ax_xt.set_title(f"{nome_metodo} - Posicao x(t)")
    ...
    def update(i):
        # Calcula o desenho da mola para a posicao atual da massa
        ...
        # Atualiza o grafico x(t) ate o instante atual
